...
import com.samples.jetsnack.model.SnackbarManager
 ...
 * 负责保存与 [JetsnackApp] 相关的状态并包含与 UI 相关的逻辑。
 */
@Stable
class JetsnackAppState(
    ...
    // ----------------------------------------------------------
    // BottomBar state source of truth 底部栏状态事实来源
    // ----------------------------------------------------------
    ...
    //读取此属性将导致在底部栏需要显示或不显示时重新组合。并非所有路线都需要显示底部栏。
    ...
    // ----------------------------------------------------------
    // Navigation state source of truth 导航状态事实来源
    // ----------------------------------------------------------
